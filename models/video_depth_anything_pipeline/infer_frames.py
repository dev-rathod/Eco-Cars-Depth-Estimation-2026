...
def parse_args():
    ...
    parser.add_argument(
        "--vda_root",
        type=Path,
        default=Path(os.environ.get("VIDEO_DEPTH_ANYTHING_ROOT", ".")),
        help="Path to a local clone of DepthAnything/Video-Depth-Anything.",
    )
...
def resolve_checkpoint(args):
    ...
    return args.vda_root / "checkpoints" / f"{checkpoint_name}_{args.encoder}.pth"
...
def build_model(encoder, metric, checkpoint_path, vda_root, device):
    sys.path.insert(0, str(vda_root.resolve()))
...
def main():
    ...
    if not args.vda_root.exists():
        raise FileNotFoundError(f"Video-Depth-Anything root not found: {args.vda_root}")
    ...
    model = build_model(args.encoder, args.metric, checkpoint_path, args.vda_root, device)
